...
const LEGACY_OAUTH_KEY: &str = "oauth";
const PROVIDER_OAUTH_KEY: &str = "oauthProviders";
...
pub fn load_oauth_credentials() -> io::Result<Option<OAuthTokenSet>> {
    load_provider_oauth_credentials(ANTHROPIC_PROVIDER_ID)
}

pub fn load_provider_oauth_credentials(provider: &str) -> io::Result<Option<OAuthTokenSet>> {
    let path = credentials_path()?;
    let root = read_credentials_root(&path)?;
    if let Some(oauth) = root
        .get(PROVIDER_OAUTH_KEY)
        .and_then(Value::as_object)
        .and_then(|providers| providers.get(provider))
    {
        return parse_stored_oauth_credentials_value(oauth);
    }
    if provider == ANTHROPIC_PROVIDER_ID {
        return root
            .get(LEGACY_OAUTH_KEY)
            .map_or(Ok(None), parse_stored_oauth_credentials_value);
    }
    Ok(None)
}

pub fn save_oauth_credentials(token_set: &OAuthTokenSet) -> io::Result<()> {
    save_provider_oauth_credentials(ANTHROPIC_PROVIDER_ID, token_set)
}
...
pub fn save_provider_oauth_credentials(
    ...
    let stored = serde_json::to_value(StoredOAuthCredentials::from(token_set.clone()))
        .map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))?;
    if provider == ANTHROPIC_PROVIDER_ID {
        root.insert(LEGACY_OAUTH_KEY.to_string(), stored);
    } else {
        let providers = ensure_provider_credentials_map(&mut root)?;
        providers.insert(provider.to_string(), stored);
    }
    write_credentials_root(&path, &root)
}

pub fn clear_oauth_credentials() -> io::Result<()> {
    clear_provider_oauth_credentials(ANTHROPIC_PROVIDER_ID)
}

pub fn clear_provider_oauth_credentials(provider: &str) -> io::Result<()> {
    let path = credentials_path()?;
    let mut root = read_credentials_root(&path)?;
    if provider == ANTHROPIC_PROVIDER_ID {
        root.remove(LEGACY_OAUTH_KEY);
    } else if let Some(providers) = root
        .get_mut(PROVIDER_OAUTH_KEY)
        .and_then(Value::as_object_mut)
    {
        providers.remove(provider);
        if providers.is_empty() {
            root.remove(PROVIDER_OAUTH_KEY);
        }
    }
    write_credentials_root(&path, &root)
}
...
fn parse_stored_oauth_credentials_value(value: &Value) -> io::Result<Option<OAuthTokenSet>> {
    if value.is_null() {
        return Ok(None);
    }
...
fn ensure_provider_credentials_map(
    root: &mut Map<String, Value>,
) -> io::Result<&mut Map<String, Value>> {
    if !matches!(root.get(PROVIDER_OAUTH_KEY), Some(Value::Object(_))) {
        root.insert(PROVIDER_OAUTH_KEY.to_string(), Value::Object(Map::new()));
    }
    root.get_mut(PROVIDER_OAUTH_KEY)
        .and_then(Value::as_object_mut)
        .ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                "provider credentials store must contain a JSON object",
            )
        })
}
...
mod tests {
    ...
    #[test]
    fn provider_oauth_credentials_round_trip_coexists_with_legacy_credentials() {
        let _guard = env_lock();
        let config_home = temp_config_home();
        std::env::set_var("CLAW_CONFIG_HOME", &config_home);
        let path = credentials_path().expect("credentials path");
        ...
        save_oauth_credentials(&anthropic).expect("save anthropic credentials");
        save_provider_oauth_credentials("copilot", &copilot).expect("save copilot credentials");

        assert_eq!(
            load_oauth_credentials().expect("load anthropic credentials"),
            Some(anthropic.clone())
        );
        assert_eq!(
            load_provider_oauth_credentials("copilot").expect("load copilot credentials"),
            Some(copilot)
        );

        let saved = std::fs::read_to_string(&path).expect("read saved file");
        assert!(saved.contains("\"oauth\""));
        assert!(saved.contains("\"oauthProviders\""));
        assert!(saved.contains("\"copilot\""));

        clear_provider_oauth_credentials("copilot").expect("clear copilot credentials");
        assert_eq!(
            load_provider_oauth_credentials("copilot").expect("load cleared copilot"),
            None
        );
        assert_eq!(
            load_oauth_credentials().expect("load anthropic after copilot clear"),
            Some(anthropic)
        );

        clear_oauth_credentials().expect("clear anthropic credentials");
        std::env::remove_var("CLAW_CONFIG_HOME");
        std::fs::remove_dir_all(config_home).expect("cleanup temp dir");
    }
