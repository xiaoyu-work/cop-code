...
fn read_git_status(cwd: &Path) -> Option<String> {
    ...
    if trimmed.len() <= MAX_GIT_STATUS_CHARS {
        return Some(trimmed.to_string());
    }
    let mut truncated = trimmed[..MAX_GIT_STATUS_CHARS].to_string();
    if let Some(last_newline) = truncated.rfind('\n') {
        truncated.truncate(last_newline);
    }
    truncated.push_str("\n... (truncated — run `git status` with BashTool for the full listing)");
    Some(truncated)
}
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if let Some(status) = &project_context.git_status {
        ...
        lines.push("Git status snapshot (this is a snapshot in time and will not update during the conversation):".to_string());
...
fn get_simple_system_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "Use standard markdown syntax for formatting: pipe tables (| col | col |), backtick code blocks, etc. Do NOT use Unicode box-drawing characters for tables.".to_string(),
...
mod tests {
    ...
    #[test]
    fn discover_with_git_does_not_embed_diff_in_prompt_context() {
        let _guard = env_lock();
        ensure_valid_cwd();
        let root = temp_dir();
        fs::create_dir_all(&root).expect("root dir");
        ...
        // System prompt should NOT include git diff — only git status.
        assert!(context.git_diff.is_none());
        let status = context.git_status.expect("git status should be present");
        assert!(status.contains("tracked.txt"));
