...
fn parse_optional_provider(root: &JsonValue) -> Result<Option<String>, ConfigError> {
    ...
    match provider {
        ...
        "copilot" => Ok(Some("copilot".to_string())),
        "openai" => Ok(Some("openai".to_string())),
...
mod tests {
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        fs::write(
            ...
            r#"{"provider":"copilot","model":"opus","permissionMode":"acceptEdits"}"#,
        )
        ...
        assert_eq!(loaded.provider(), Some("copilot"));
