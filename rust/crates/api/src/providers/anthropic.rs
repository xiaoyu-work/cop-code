...
use super::{
    model_token_limit, preflight_message_request, resolve_model_alias, Provider, ProviderFuture,
};
...
impl AnthropicClient {
    ...
    async fn preflight_message_request(&self, request: &MessageRequest) -> Result<(), ApiError> {
        ...
        // Fall back to local heuristic when the count-tokens API is unavailable.
        let Ok(counted_input_tokens) = self.count_tokens(request).await else {
            return preflight_message_request(request);
        };
