...
#[must_use]
pub fn normalize_model_for_provider(provider: ProviderKind, model: &str) -> String {
    let trimmed = model.trim();
    match provider {
        ProviderKind::Copilot => match trimmed.to_ascii_lowercase().as_str() {
            "opus" | "claude-opus-4-6" => "claude-opus-4.6".to_string(),
            "sonnet" | "claude-sonnet-4-6" => "claude-sonnet-4.6".to_string(),
            "haiku" | "claude-haiku-4-5-20251213" => "claude-haiku-4.5".to_string(),
            _ => trimmed.to_string(),
        },
        ProviderKind::Anthropic | ProviderKind::Xai => resolve_model_alias(trimmed),
...
pub const fn default_model_for_provider(provider: ProviderKind) -> &'static str {
    match provider {
        ...
        ProviderKind::Copilot => "claude-sonnet-4.6",
...
pub fn detect_provider_kind(model: &str) -> ProviderKind {
    ...
    if copilot::has_github_token_from_env_or_saved().unwrap_or(false) {
...
mod tests {
    ...
    use super::{
        default_model_for_provider, detect_provider_kind, max_tokens_for_model, model_token_limit,
        normalize_model_for_provider, preflight_message_request, resolve_model_alias, ProviderKind,
    };
    ...
    #[test]
    fn normalizes_copilot_specific_claude_aliases() {
        assert_eq!(
            normalize_model_for_provider(ProviderKind::Copilot, "sonnet"),
            "claude-sonnet-4.6"
        );
        assert_eq!(
            normalize_model_for_provider(ProviderKind::Copilot, "claude-opus-4-6"),
            "claude-opus-4.6"
        );
    }

    #[test]
    fn provider_kind_parsing_and_defaults_cover_copilot() {
        assert_eq!(ProviderKind::parse("copilot"), Some(ProviderKind::Copilot));
        assert_eq!(
